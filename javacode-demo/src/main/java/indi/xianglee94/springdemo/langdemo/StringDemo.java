package indi.xianglee94.springdemo.langdemo;
...
// 3. 字符串直接常量相加的时候，例如: String c = "aa" + "bb";  其中的aa/bb只要有任何一个不是字符串字面常量形式，都不会在常量池生成"aabb". 且此时jvm做了优化，不//   会同时生成"aa"和"bb"在字符串常量池中
    ...
    static void test7() {
        String s1 = null, s2 = null, s3 = null, s4 = null, temp = null;

        temp = "hh";
        s1 = "a" + temp;// 如果调用s1.intern 则最终返回
        s2 = "ahh";
        System.out.println(s1 == s2);    //
    ...
    static void test9() {
        ...
        s3 = new String("1") + new String("1");// 此时生成了四个对象 常量池中的"1" + 2个堆中的"1" + s3指向的堆中的对象（注此时常量池不会生成"11"）
        String s5 = new String("1") + new String("1");
        s5.intern();
        s3.intern();    // jdk1.7之后，常量池不仅仅可以存储对象，还可以存储对象的引用，会直接将s3的地址存储在常量池
        s4 = "11";    // jdk1.7之后，常量池中的地址其实就是s3的地址
